...
test.describe("scoring", () => {
  ...
  const playerScore = (page: import("@playwright/test").Page) =>
    page.locator(".score-chip").filter({ hasText: "Aravind" }).locator(".score-value");

  test("award gives the cell's points to a player; undo reverts it", async ({ page }) => {
    await page.goto("/moderator?transport=ws");

    await expect(playerScore(page)).toHaveText("0");

    // Open the top-left 50-point cell and award it to Aravind.
    await page.getByRole("button", { name: "50", exact: true }).first().click();
    await page.getByRole("dialog").getByRole("button", { name: "Aravind", exact: true }).click();

    await expect(playerScore(page)).toHaveText("50");
